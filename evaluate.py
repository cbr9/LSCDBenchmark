...
[_, annotation, uses, annotators, output_file] = sys.argv
...
with open(uses, encoding='utf-8') as csvfile: 
    ...
    try:    
        identifier2identifier = {row['identifier_system']:row['identifier'] for row in uses}    
    except KeyError:
        identifier2identifier = {row['identifier']:row['identifier'] for row in uses}    
...
annotation = [(identifier2identifier[row['identifier1']],identifier2identifier[row['identifier2']],float(row['judgment']),row['comment'],user2annotator[row['annotator']]) for row in annotation]
